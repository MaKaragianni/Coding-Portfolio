...
class EnhancedLibrarySystem:
    ...
    def user_login(self):
        ...
        if user_id:
            ...
            print("User not found. Please check your email or register.")
            return None
...
if __name__ == "__main__":
    ...
    user_id = lib_system.user_login()
    if user_id:
        lib_system.process_borrow(user_id)
    else:
        print("Borrowing process cannot proceed without login.")

    # User registration
    user_id = lib_system.process_borrow(user_id)
